...
const stub = createServer(async (req, res) => {
  ...
  res.end(JSON.stringify({ method: req.method, url: req.url, authorization: req.headers.authorization ?? null,
...
try {
  ...
  let result = await fetch(`${origin}/api/echo?q=one%20two`, { headers: { authorization: "Bearer forged", cookie: "unrelated=private" } });
  const echo = await result.json();
  assert.equal(new URL(echo.url, origin).searchParams.get("q"), "one two");
  assert.deepEqual({ ...echo, url: "/api/echo" }, { method: "GET", url: "/api/echo", authorization: null, cookie: null, body: "" });
  ...
  result = await fetch(`${origin}/api/echo`, { method: "POST", headers: {
    origin, "content-type": "application/json", authorization: "Bearer forged", cookie: "unrelated=private",
  }, body: '{"action":"review"}' });
  assert.deepEqual(await result.json(), { method: "POST", url: "/api/echo", authorization: null,
  ...
  console.log("PASS: runtime API_URL, liveness, headers, origin checks, JSON/HEAD/204, size limits and API outage.");
} finally {
