      ...
      <section className="panel" style={{ width: "min(720px, 96vw)", maxHeight: "86vh", overflow: "auto", padding: 24 }} onClick={event => event.stopPropagation()}>
        ...
        {!model ? <div className="empty"><strong>Model unavailable</strong>The packaged scikit-learn artifact could not be loaded.</div> : <>
          <div className="cards" style={{ marginTop: 20 }}>
            <div className="card"><span className="lbl">Accuracy</span><div className="num">{percentage(metrics?.accuracy)}</div><small>Cross-validated</small></div>
            <div className="card"><span className="lbl">Precision</span><div className="num">{percentage(metrics?.precision)}</div><small>Spam class</small></div>
            <div className="card"><span className="lbl">Recall</span><div className="num">{percentage(metrics?.recall)}</div><small>Spam class</small></div>
            <div className="card"><span className="lbl">F1 score</span><div className="num">{percentage(metrics?.f1)}</div><small>Spam class</small></div>
          </div>
          {!metrics && <p className="muted">Evaluation metrics were not recorded in this artifact. They will be included after the next training run.</p>}
