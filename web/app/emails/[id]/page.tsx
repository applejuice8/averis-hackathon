...
export default async function EmailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    {r?.review_reason && <div className="hero-note bad"><strong>{REASONS[r.review_reason] ?? r.review_reason}</strong><span className="muted">Inspect the attachments before making a decision.</span></div>}
