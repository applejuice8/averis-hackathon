...
export default function ReviewActions({ resultId, status, defectFields, canCompare }: { resultId: string; status: string; defectFields: string[]; canCompare: boolean }) {
  ...
  return <section className="panel review-form"><h2>Reviewer decision</h2><p className="muted">Confirm the finding or correct it after checking the source documents.</p><div className="filters"><input aria-label="Reviewer name" placeholder="Reviewer name (optional)" value={reviewer} onChange={e => setReviewer(e.target.value)} maxLength={100}/></div><div className="actions"><button disabled={busy} onClick={() => act("confirm")}>Confirm verdict</button>{status !== "OK" && <button className="ghost" disabled={busy} onClick={() => act("override_status", {status:"OK",review_reason:null})}>Mark clear</button>}{status !== "NEEDS_REVIEW" && <button className="ghost" disabled={busy} onClick={() => act("override_status", {status:"NEEDS_REVIEW",review_reason:"manual_flag"})}>Escalate to review</button>}</div>
    {canCompare && <details><summary>Correct the mismatch fields</summary><p className="muted">Select the fields that differ. Saving no fields marks the result clear.</p><div className="field-options">{COMPARE_FIELDS.map(f => <label key={f}><input type="checkbox" checked={fields.includes(f)} disabled={busy} onChange={e => setFields(previous => e.target.checked ? [...previous,f] : previous.filter(v => v !== f))}/>{FIELDS[f]}</label>)}</div><button className="ghost" disabled={busy} onClick={() => act("override_fields", {defect_fields:fields})}>Save field corrections</button></details>}
