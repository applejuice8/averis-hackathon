...
export default async function Dashboard() {
  ...
  return <>
    ...
    <div className={`hero-note${review.length ? " bad" : ""}`}><span className="hero-icon" aria-hidden="true">◎</span><div><strong>{review.length ? `${review.length} cases need a second look` : "Every detail deserves a clear decision"}</strong><p>Missing files, unreadable documents, and incomplete details go to human review.</p></div><Link href="/review" className="text-link">Review cases →</Link></div>
