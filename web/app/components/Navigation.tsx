...
export default function Navigation() {
  ...
  return <aside className="sidebar">
    ...
    <nav aria-label="Main navigation">
      {[["/", "Overview", "◫"], ["/inbox", "Inbox", "▤"], ["/calendar", "Calendar", "□"], ["/review", "Human review", "◎"], ["/runs", "Pipeline runs", "↗"]].map(([href, name, icon]) => <Link key={href} href={href} className={(href === "/" ? path === href : path.startsWith(href)) ? "active" : ""} aria-current={path === href ? "page" : undefined}><span aria-hidden="true">{icon}</span>{name}</Link>)}
    </nav>
    <ReviewerPanel />
